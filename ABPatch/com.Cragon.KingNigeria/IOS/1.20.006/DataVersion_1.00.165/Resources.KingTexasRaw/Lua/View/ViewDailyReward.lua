...
function ViewDailyReward:onCreate()
	ViewHelper:PopUi(self.ComUi,"签到奖励")
	self.CasinosContext = CS.Casinos.CasinosContext.Instance
	self.MapDailyReward = {}
    self.GBtnGetReward = self.ComUi:GetChild("BtnGetReward").asButton
    self.GBtnGetReward.onClick:Add(
		function()
			self:onClickBtnGetReward()
		end
	)
	local btn_beVip = self.ComUi:GetChild("BtnBeVip").asButton
...
function ViewDailyReward:onClickBtnGetReward()
	...
	then
		CS.Casinos.UiSoundMgr.Instant:playSound(self.REWARD_SOUND, self.GoUi, true)
