...
function ViewShop:onClickBtnVip()
	...
	then
		...
		tips = self.ViewMgr.LanMgr:getLanValue("ShopVipTips")
		tips = string.format(self.ViewMgr.LanMgr:getLanValue("ShopVipTips"), next_needtotalexp)
	end
...
function ViewShop:createChip()
	...
	for key,value in pairs(map_tbitem) do
		...
		then
			if (tb_item.PriceType == PriceType.Gold)
			then
