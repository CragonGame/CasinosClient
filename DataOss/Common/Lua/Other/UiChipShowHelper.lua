...
function UiChipShowHelper:getGoldShowStr(gold, lan_base, show_short_way1, precision_length1)
    ...
    if (precision_length1 ~= nil) then
        precision_length = precision_length1
    end

    if (lan_base == nil) then
        if (show_short_way == false) then
            ...
            for i = multiple, 1, -1 do
                if (remainder == 0 and i == multiple) then
                else
                    local index = 0
                    if (remainder == 0) then
            ...
            if (gold_thousand_multiple > 0 and gold_thousand_multiple < 10) then
