...
function ViewDesktopHResult:onUpdate(tm)
    self.Tm = self.Tm - tm
    if (self.Tm <= self.ShowTm - 1)
    then
        self.ShowTm = self.Tm
        self:_setCloseTime()
    end
...
function ViewDesktopHResult:_setCloseTime()
    self.GTextCloseTips.text = string.format(self.ViewMgr.LanMgr:getLanValue("CloseAuto"),
            tostring(math.ceil(self.ShowTm)))
end
