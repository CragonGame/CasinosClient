    ...
    if (self.PlayerInfo.Player.IsMe == true and self.CardFirst ~= nil) then
        ...
        if (self.SeatWidget.GLoaderCardFirst.texture == nil or is_same == false) then
            if (CS.System.String.IsNullOrEmpty(me_first_card_name) == false) then
                self.LoaderTicket1 = self.CasinosContext.TextureMgr:getTexture(l_me_first_card_name, self.CasinosContext.PathMgr.DirAbCard .. l_me_first_card_name .. ".ab",
                        function(ticket, t)
                            if (self.SeatWidget ~= nil) then
        ...
        then
            if (CS.System.String.IsNullOrEmpty(me_second_card_name) == false) then
                self.LoaderTicket2 = self.CasinosContext.TextureMgr:getTexture(l_me_second_card_name, self.CasinosContext.PathMgr.DirAbCard .. l_me_second_card_name .. ".ab",
                        function(ticket, t)
                            if (self.SeatWidget ~= nil) then
...
function PlayerSeatWidgetControllerTexas:_resetCard(hide_card)
    ...
    if self.TweenerRotate1 ~= nil then
        self.TweenerRotate1:Kill(false)
    end
    if self.TweenerRotate2 ~= nil then
        self.TweenerRotate2:Kill(false)
    end
    self.LoaderTicket1 = nil
    self.LoaderTicket2 = nil
    if (self.SeatWidget.GLoaderCardFirst ~= nil) then
