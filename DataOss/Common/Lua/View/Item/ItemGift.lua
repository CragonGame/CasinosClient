...
-- 显示上是一格Item，不仅仅是礼物。目前背包，礼物商城，他人资料中在使用。从ViewPool中获取。
...
function ItemGift:SetGift(gift_id, is_buygift, is_mine, to_etguid, from_name, gift_belong, item)
    ...
    if tb_gift.UnitType == "Consume" then
        ...
        self.GLoaderIcon.icon = self.CasinosContext.PathMgr.DirAbItem .. string.lower(tb_gift.Icon) .. ".ab"
    end

    if (self.GTextGiftName ~= nil) then
    ...
    if (self.GTextGiftValue ~= nil) then
        ...
        elseif (tb_gift.PriceType == PriceType.Point) then
            -- 积分
            price = price .. self.LanMgr:getLanValue("Point")
        end
        self.GTextGiftValue.text = price
    end
    if (item ~= nil and item.ItemData ~= nil) then
...
function ItemGift:_onClick()
    ...
    local tb_item = self.CasinosContext.TbDataMgrLua:GetData("Item", self.ItemId)
