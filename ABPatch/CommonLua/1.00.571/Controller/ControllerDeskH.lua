...
function ControllerDeskH:onHandleEv(ev)
    ...
    elseif (ev.EventName == "EvDesktopHClickBetOperateType")
    then
    ...
    elseif (ev.EventName == "EvDesktopHClickStandPlayerBtn")
    then
    ...
    elseif (ev.EventName == "EvEntityPlayerEnterDesktop")
    then
...
function ControllerDeskH:clearDesktopHundred(need_createmainui)
    ...
    then
        ...
        --[[if (self.ControllerPlayer.IsTopStar)
        then
            self.CasinosContext.Instance:play(self.CasinosContext.Listener.BackSound, CS.Casinos._eSoundLayer.Background)
        end]]--
    end
...
function ControllerDeskH:playerLeaveDesktopH(map_param)
    ...
    local controller_lobby = self.ControllerMgr:GetController("ControllerLobby")
