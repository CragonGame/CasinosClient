...
function ViewHandCardTexas:showCard(delay_tm)
    if self.CancelShowCard then
        ...
        self.TweenerRotate = CS.FairyGUI.GTween.To(self.GLoaderCard.rotationY, 90, UiCardCommonEx.RotateTime):SetTarget(self.GLoaderCard, CS.FairyGUI.TweenPropType.RotationY):SetEase(CS.FairyGUI.EaseType.Linear):OnComplete(
                function()
            ...
            if (CS.System.String.IsNullOrEmpty(card_resouce_name) == false) then
                self.LoaderTicket = CS.Casinos.CasinosContext.Instance.TextureMgr:getTexture(l_card_resouce_name, CS.Casinos.CasinosContext.Instance.PathMgr:combinePersistentDataPath(CS.Casinos.UiHelperCasinos.getABCardResourceTitlePath() .. l_card_resouce_name .. ".ab"),
                        function(ticket, t)
                            ...
                            self.TweenerRotate = CS.FairyGUI.GTween.To(self.GImageCardBack.rotationY, 90, UiCardCommonEx.RotateTime):SetTarget(self.GImageCardBack, CS.FairyGUI.TweenPropType.RotationY):SetDelay(delay_tm):SetEase(CS.FairyGUI.EaseType.Linear):OnComplete(
                                    function()
                                        ViewHelper:setGObjectVisible(false, self.GImageCardBack)
                                        ViewHelper:setGObjectVisible(true, self.GLoaderCard)
                                        self.GImageCardBack.rotationY = 180
                                        self.GLoaderCard.rotationY = 90
                                        self.TweenerRotate = CS.FairyGUI.GTween.To(self.GLoaderCard.rotationY, 0, UiCardCommonEx.RotateTime):SetTarget(self.GLoaderCard, CS.FairyGUI.TweenPropType.RotationY):SetEase(CS.FairyGUI.EaseType.Linear):OnComplete(
                                                function()
