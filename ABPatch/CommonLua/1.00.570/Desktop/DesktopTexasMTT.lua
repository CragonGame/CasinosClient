...
function DesktopTexasMTT:DesktopUser(method_id, method_data)
    ...
    then
        ...
        if player_info.PlayerGuid == self.ControllerDesktop.Guid then
            ...
            self:refreshBtnRebuyAddOnStat()
            ViewHelper:UiShowMsgBox(self.ControllerDesktop.ControllerMgr.ViewMgr.LanMgr:getLanValue("RebuySuccess"),self.BDesktopSnapshotMatchTexas.RaiseBlindTbInfo.AddonScore) -- "RebuyTip1"  "AddonSuccess"
        else
            --local view_mgr = self.ControllerDesktop.ControllerMgr.ViewMgr
