	...
	then
		...
		then
			...
			then
				...
				if (player_info.PlayerInfoMore.OnlineState == CS.Casinos.PlayerOnlineState.Offline)
				then
...
function ViewLobby:initPlayerInfo()
	...
	text_name.text = self.ControllerActor.PropNickName:get()
...
function ViewLobby:requestLobbyDesk()
	...
	dektop_filter.FactoryName = CS.Casinos.CasinosModule.Texas:ToString()
...
function ViewLobby:setCurrentPlayingFriend(list_playerinfo)
	...
	then
		self.GTextNoFriendTips.text = self.ViewMgr.LanMgr:getLanValue("没有好友打牌", "NoFriendPlayCard")
...
function ViewLobby:onClickBtnPersonInfo()
	...
	then
		...
		if (a == scroll_index)
		then
			local tips = self.ViewMgr.LanMgr:getLanValue("当前已选择合适打牌桌", "HasSelectedRightTable")
...
function ViewLobby:onBetChipOperateListScrollEnd()
	...
	if (can_play == false)
	then
		tips = self.ViewMgr.LanMgr:getLanValue("没有足够的筹码，不能在这里玩", "NoEnoughChipCanotPlay")
	end
...
function ViewLobby:onSearchFilterChanged()
	...
	then
		filter_tips = self.ViewMgr.LanMgr:getLanValue("任何座位", "AnySeat")
	elseif(player_num == TexasDesktopSeatNum.Five)
	then
		filter_tips = self.ViewMgr.LanMgr:getLanValue("5座", "Five")
	elseif(player_num == TexasDesktopSeatNum.Nine)
	then
		filter_tips = self.ViewMgr.LanMgr:getLanValue("9座", "Nine")
	end
	self.GTextSeatType.text = filter_tips
	local show_fullDesk = self.DeskSearchFilter.is_seat_full
	if(show_fullDesk)
	then
		filter_tips = self.ViewMgr.LanMgr:getLanValue("显示", "Show")
	else
		filter_tips = self.ViewMgr.LanMgr:getLanValue("隐藏", "Hide")
	end
