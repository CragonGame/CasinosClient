...
function ViewPlayerInfo:onCreate()
	...
	local wechat_id = self.ControllerActor.PropWeChatId:get()
	local group = self.ComUi:GetChild("WeChatBlinded").asGroup
	local select_index = 0
	if (CS.Casinos.CasinosContext.Instance.LoginType == CS.Casinos._eLoginType.WeiXin)
	then
		group = self.ComUi:GetChild("BlindWeChat").asGroup
		select_index = 1
	end
	self.ControllerBtn = self.ComUi:GetController("ControllerBtn")
	self.ControllerBtn = select_index
