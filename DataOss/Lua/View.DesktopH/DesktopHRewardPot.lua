...
function DesktopHRewardPot:setSysPumpingGold(pot_index)        
            ...
            then
                ...
                if (pot_index ~= DesktopHBankPlayer.BankerIndex)
                then
                    auto_destroy = false
                    gold_fix_pos = true
...
function DesktopHRewardPot:showLooseGoldAni(pot_index, win_rewardpot_golds)        
            ...
            then
				...
				for k,v in pairs(self.ViewDesktopH:getDesktopHChairAll()) do
					 if (v.SeatPlayerInfoHundred ~= nil)
					    then
					        if (v.SeatPlayerInfoHundred.PlayerInfoCommon.
					        PlayerGuid ~= self.ViewDesktopH.ControllerPlayer.Guid)
							then
            ...
            if (self.CurrentTotalRewardPotGolds ~= self.CurrentNeedShowRewardPotGolds)
            then
...
function DesktopHRewardPot:setRewardGolds1(reward_golds)
            ...
            if (self.CurrentTotalRewardPotGolds < 0)
            then
...
function DesktopHRewardPot:_cancelTask()        
            if (self.FTaskerSetPumpingGold ~= nil)
            then
