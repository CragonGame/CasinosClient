...
function ViewPlayerInfo:onCreate()
    ...
    local btn_login = self.ComUi:GetChild("BtnLogin").asButton
    ...
    local btn_gift = self.ComUi:GetChild("Gift").asCom
    btn_gift.onClick:Add(
		function()
			self:onClickGiftShop()
		end
	)
    local btn_property = self.ComUi:GetChild("Property").asCom
    ...
    local btn_moreCoin = self.ComUi:GetChild("MoreCoins").asButton
    btn_moreCoin.onClick:Add(
		function()
			self:onClickMoreCoin()
		end
	)
    local btn_inviteFriend = self.ComUi:GetChild("InviteFriend").asButton
    ...
    local nickname_tips = "点击昵称开始编辑"
	if (self.CasinosContext.UseLan)
	then
		nickname_tips = self.ViewMgr.LanMgr:getLanValue("","ClickNickname")
	end
	...
	self.ChangeNameNeedItemTbId = TbDataHelper:GetCommonValue(CS.Casinos._eTbDataCommonKey.ChangeNameItemId:ToString())
	...
	then
		...
		then
            ...
            if (self.CasinosContext.UseLan)
			then
				f = self.ViewMgr.LanMgr:getLanValue("","NeedRenameCard")
			end
    ...
    self.HornItemTbId = TbDataHelper:GetCommonValue(CS.Casinos._eTbDataCommonKey.IMMarqueeCostItemId)
    ...
    ev.factory_name = CS.Casinos.CasinosModule.Texas:ToString()
...
function ViewPlayerInfo:onHandleEv(ev)
	...
	then
        ...
        local tips = self.ViewMgr.LanMgr:getLanValue("更换头像成功!", "IconSuccess")
...
function ViewPlayerInfo:initPlayerInfo()
    ...
    self.GInputNickName.text = self.ControllerActor.PropNickName:get()
	...
	self.GTextAddress.text = self.ViewMgr.LanMgr:getLanValue("地址", "Address") .. " " .. address
    self:setGift()
end
...
function ViewPlayerInfo:changeNickName()
	...
	then
		...
		then
			...
			then
				ViewHelper:UiShowMsgBox(self.ViewMgr.LanMgr:getLanValue("改名卡不足，请到商城购买！", "RenameCardInsufficient"),
				function()
		...
		then
			ViewHelper:UiShowInfoFailed(self.ViewMgr.LanMgr:getLanValue("昵称不可为空！", "NicknameNotNull"))
            return
		end
        ViewHelper:UiBeginWaiting(self.ViewMgr.LanMgr:getLanValue("更改信息中...", "ChangeInfo"))
