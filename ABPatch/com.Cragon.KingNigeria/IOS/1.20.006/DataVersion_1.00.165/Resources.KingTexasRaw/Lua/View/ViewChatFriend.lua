...
function ViewChatFriend:onCreate()
	ViewHelper:PopUi(self.ComUi,"消息")
...
function ViewChatFriend:onHandleEv(ev)
	...
	then
		...
		if (self.CurrentChatTarget ~= nil and player_info.PlayerInfoCommon.PlayerGuid == self.CurrentChatTarget.PlayerInfoCommon.PlayerGuid and player_info.PlayerInfoMore.OnlineState == CS.Casinos.PlayerOnlineState.Offline)
		then
...
function ViewChatFriend:onClickBtnSendMsg()
	...
	then
		ViewHelper:UiShowInfoFailed(self.ViewMgr.LanMgr:getLanValue("发送内容不可为空!", "SendNoEmpty"))
		return
	end
