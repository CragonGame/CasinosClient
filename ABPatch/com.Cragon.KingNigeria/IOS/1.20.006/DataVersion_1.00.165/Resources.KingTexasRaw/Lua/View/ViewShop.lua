...
function ViewShop:onCreate()
    ...
    local btn_levelupVip = self.ComUi:GetChild("BtnVIPLevelUp").asButton
	...
	local btn_editAddress = self.ComUi:GetChild("BtnAddress").asButton
...
function ViewShop:onClickBtnVip()
	...
	then
		...
		tips = self.CasinosContext:parseLanStr("再充值%sRMB即可提升为","ShopVipTips")
        tips = string.format(self.CasinosContext:parseLanStr("再充值%sRMB即可提升为","ShopVipTips"), next_needtotalexp)
	end
...
function ViewShop:createDiamond()
	if(LuaHelper:GetTableCount(self.MapShopDiamond) > 0)
	then
		return
	end
...
function ViewShop:createChip()
	if(LuaHelper:GetTableCount(self.MapShopGold)> 0)
	then
		return
	end
    local map_tbitem = self.CasinosContext.TbDataMgrLua:GetMapData("Item")
...
function ViewShop:createConsume()
	if (LuaHelper:GetTableCount(self.MapShopItemCosume) > 0)
	then
		return
	end
    local map_tbitem = self.CasinosContext.TbDataMgrLua:GetMapData("Item")
	for key,value in pairs(map_tbitem) do
		local tb_item = value
        if (tb_item.UnitType == "Consume" and tb_item.Id ~= self.TemporaryHideItemId)
		then
...
function ViewShop:createVIP()
	if(LuaHelper:GetTableCount(self.MapShopItemVip) > 0)
	then
		return
	end
...
function ViewShop:setListGoods(goodsType)
	...
	if(goodsType == GoodsType.Gift1)
	then
		self.GControllerTabGift:SetSelectedIndex(0)
		local map_tbitem = self.CasinosContext.TbDataMgrLua:GetMapData("Item")
		for key,value in pairs(map_tbitem) do
			local tb_item = value
			if(tb_item.UnitType == "GoodsVoucher")
			then
				local com = self.GListGoods:AddItemFromPool().asCom
				local discirbe = {}
				discirbe[1] = "[size=25]\n[/size]" -- 空25像素
				discirbe[2] = "[size=25]"
				discirbe[3] = tb_item.Desc
				discirbe[4] = "[/size]\n[size=18][color=#A4BCEE]兑换券 x1[/color][/size]"
				local url = CS.Casinos.CasinosContext.Instance.PathMgr:combinePersistentDataPath
				...
				local item = ItemUiShopGoods:new(nil,com,table.concat(discirbe),url,btn_title,
					function()
						...
						local list = controller_bag.ListItemConsume
						for i = 1,#list do
							if(list[i].TbDataItem.Id == tb_item.Id)
							then
								--请求使用实物兑换券兑换实物
		                        ...
		                        ev.ItemObjId = list[i].ItemData.item_objid
	...
	elseif(goodsType == GoodsType.Gift2)
	then
		self.GControllerTabGift:SetSelectedIndex(1)
	elseif(goodsType == GoodsType.Gift3)
	then
		self.GControllerTabGift:SetSelectedIndex(2)
		local map_tbitem = self.CasinosContext.TbDataMgrLua:GetMapData("Item")
		for key,value in pairs(map_tbitem) do
			local tb_item = value
		    if (tb_item.PriceType == 4)
			then
				local list_dataChip = {}
				local list_dataCertificate = {}
				local com = self.GListGoods:AddItemFromPool().asCom
				if(tb_item.UnitType == "GoldPackage")
				then
					...
					local url = self.CasinosContext.PathMgr:combinePersistentDataPath(CS.Casinos.UiHelperCasinos:getABItemResourceTitlePath() .. tostring(tb_item.Icon) .. ".ab")
					local btn_title = "兑换"
					local item = ItemUiShopGoods:new(nil,com,table.concat(discirbe),url,btn_title,
						function()
							--请求使用积分兑换筹码
				...
				elseif(tb_item.UnitType == "GoodsVoucher")
				then
					local discirbe = {}
					discirbe[1] = "[size=25]\n[/size]" -- 空25像素
					discirbe[2] = "[size=25]"
