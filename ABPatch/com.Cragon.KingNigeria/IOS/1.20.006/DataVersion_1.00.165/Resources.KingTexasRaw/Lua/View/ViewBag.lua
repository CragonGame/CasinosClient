...
function ViewBag:onCreate()
	ViewHelper:PopUi(self.ComUi,"背包")
...
function ViewBag:onHandleEv(ev)
	if(ev ~= nil)
	then
		if(ev.EventName == "EvEntityBagDeleteItem")
		then
			self:updateBag()
...
function ViewBag:rendererGiftNormal(index,item)
	if (self.GControllerBag.selectedIndex == 0)
	then
		if (#self.ControllerBag.ListItemConsume > index)
		then
			local gift = self.ControllerBag.ListItemConsume[index + 1]
			if(gift.UnitLink.UnitType == "GoldPackage") -- 自动使用金币袋子
			then
		...
		if (#self.ControllerBag.ListItemGiftNormal > index)
		then
...
function ViewBag:showTips()
    if (self.GControllerBag.selectedIndex == 0 and #self.ControllerBag.ListItemConsume == 0)
	then
		self.ControllerTips.selectedIndex = 2
	elseif(self.GControllerBag.selectedIndex == 1 and #self.ControllerBag.ListItemGiftNormal == 0)
	then
