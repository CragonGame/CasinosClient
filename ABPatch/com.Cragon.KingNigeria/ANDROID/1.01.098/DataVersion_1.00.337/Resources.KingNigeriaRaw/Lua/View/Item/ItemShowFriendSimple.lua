...
function ItemShowFriendSimple:setFriendInfo(view_friend,friend_item,is_friend)
	...
	self.GTextBtnNickName.text = CS.Casinos.UiHelper.addEllipsisToStr(self.FriendInfo.PlayerInfoCommon.NickName,10,4)
...
function ItemShowFriendSimple:onClickComFriendItem()
	if (self.IsFriend)
	then
		if (self.ViewFriend:getCurrentFriendInfo().PlayerInfoCommon.PlayerGuid == self.FriendInfo.PlayerInfoCommon.PlayerGuid)
		then
			return
		end
		self.ViewFriend:setCurrentFriend(self.FriendInfo)
	else
		if (self.ViewFriend:getCurrentRecommandInfo().PlayerInfoCommon.PlayerGuid == self.FriendInfo.PlayerInfoCommon.PlayerGuid)
		then
			return
		end
