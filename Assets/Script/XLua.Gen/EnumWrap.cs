...
namespace XLua.CSObjectWrap
{
    ...
    public class CasinosDirTypeWrap
    {
		public static void __Register(RealStatePtr L)
        {
		    ObjectTranslator translator = ObjectTranslatorPool.Instance.Find(L);
		    Utils.BeginObjectRegister(typeof(Casinos.DirType), L, translator, 0, 0, 0, 0);
			Utils.EndObjectRegister(typeof(Casinos.DirType), L, translator, null, null, null, null, null);
			
			Utils.BeginClassRegister(typeof(Casinos.DirType), L, null, 3, 0, 0);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "Raw", Casinos.DirType.Raw);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "Resources", Casinos.DirType.Resources);
            
			Utils.RegisterFunc(L, Utils.CLS_IDX, "__CastFrom", __CastFrom);
            
            Utils.EndClassRegister(typeof(Casinos.DirType), L, translator);
        }
        ...
        static int __CastFrom(RealStatePtr L)
		{
            ...
            if (lua_type == LuaTypes.LUA_TNUMBER)
            {
                translator.PushCasinosDirType(L, (Casinos.DirType)LuaAPI.xlua_tointeger(L, 1));
            }
			
            else if(lua_type == LuaTypes.LUA_TSTRING)
            {
			    if (LuaAPI.xlua_is_eq_str(L, 1, "Raw"))
                {
                    translator.PushCasinosDirType(L, Casinos.DirType.Raw);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "Resources"))
                {
                    translator.PushCasinosDirType(L, Casinos.DirType.Resources);
                }
				else
                {
                    return LuaAPI.luaL_error(L, "invalid string for Casinos.DirType!");
                }
            }
			
            else
            {
                return LuaAPI.luaL_error(L, "invalid lua type for Casinos.DirType! Expect number or string, got + " + lua_type);
            }
