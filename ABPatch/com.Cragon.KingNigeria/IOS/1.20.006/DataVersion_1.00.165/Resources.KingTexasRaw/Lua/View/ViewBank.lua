...
function ViewBank:onCreate()
	ViewHelper:PopUi(self.ComUi,"保险箱")
...
function Bank:new(o,safe_box,group,takeout_putin_pergold)
	...
	local put_in = com:GetChildInGroup(group, "BtnPutIn")
	if (put_in ~= nil)
	then
	...
	local take_out = com:GetChildInGroup(group, "BtnTakeOut")
	if (take_out ~= nil)
	then
...
function Bank:onClickPutIn()
    ...
    if (input_num <= 0)
	then
		ViewHelper:UiShowInfoSuccess(self.ViewBank.ViewMgr.LanMgr:getLanValue("存入金额不能为0哦~", "DepositTips"))
        return
	else
		if (input_num * self.TakeOutOrPutInPerGold > self.ViewBank.ControllerActor.PropGoldAcc:get())
		then
			ViewHelper:UiShowInfoSuccess(self.ViewBank.ViewMgr.LanMgr:getLanValue("当前活动金额不足~", "DepositTips1"))
            return
		end
	end
...
function Bank:onClickTakeOut()
    ...
    if (takeout_num <= 0)
	then
		 ViewHelper:UiShowInfoSuccess(self.ViewBank.ViewMgr.LanMgr:getLanValue("取出金额不能为0哦~", "GetoutAmountTips"))
         return
	else
		if (takeout_num * self.TakeOutOrPutInPerGold > self.ViewBank.ControllerActor.PropGoldBank:get())
		then
			ViewHelper:UiShowInfoSuccess(self.ViewBank.ViewMgr.LanMgr:getLanValue("没有足够的金额可以取出哦~", "GetoutAmountTips1"))
            return
		end
	end
...
function SendGold:onClickSend()
	...
	if(n == nil)
	then
		ViewHelper:UiShowInfoSuccess(self.ViewBank.ViewMgr.LanMgr:getLanValue("请输入数字", "EnterNumTips"))
        return
	else
		send_gold = n
		if(send_gold <= 0)
		then
			ViewHelper:UiShowInfoSuccess(self.ViewBank.ViewMgr.LanMgr:getLanValue("请输入赠送金额", "SendGoldTips"))
			return	
		end
	end
	if((self.GInputTargetId.text == nil or #self.GInputTargetId.text <= 0) or self.GInputTargetId.text == "0")
	then
		ViewHelper:UiShowInfoSuccess(self.ViewBank.ViewMgr.LanMgr:getLanValue("请输入ID", "EnterIdTips"))
        return
	end
...
function ExchangeCode:onClickExchange()
	...
	then
		ViewHelper:UiShowInfoSuccess(self.ViewBank.ViewMgr.LanMgr:getLanValue("卡号不可为空", "EnterCardNumTips"))
        return
	end
	if(self.GInputExchangeCodePwd.text == nil or #self.GInputExchangeCodePwd.text <= 0)
	then
		ViewHelper:UiShowInfoSuccess(self.ViewBank.ViewMgr.LanMgr:getLanValue("密码不可为空", "EnterPwdTips"))
        return
	end
