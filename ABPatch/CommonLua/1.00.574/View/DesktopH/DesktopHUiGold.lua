...
function DesktopHUiGold:initMove(from, to, move_time,
    ...
    local from1 = CS.UnityEngine.Vector2()
    if (fix_pos)
    then
        from1.x = from.x - self.GCoGold.width / 2
        from1.y = from.y - self.GCoGold.height / 2
    else
        from1.x = from.x
        from1.y = from.y
    end
    self.GCoGold.xy = from1
    local to1 =  CS.UnityEngine.Vector2()
    to1.x = to.x - self.GCoGold.width / 2
    to1.y = to.y - self.GCoGold.height / 2
...
function DesktopHUiGold:setPostion(pos)
    local pos1 = CS.UnityEngine.Vector2()
    pos1.x = pos.x - self.GCoGold.width / 2
    pos1.y = pos.y - self.GCoGold.height / 2
    self.GCoGold.xy = pos1
end
