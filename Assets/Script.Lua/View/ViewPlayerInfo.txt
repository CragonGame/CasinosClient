...
function ViewPlayerInfo:OnCreate()
    ...
    local ev = self:GetEv("EvRequestGetPlayerModuleData")
    if (ev == nil) then
...
function ViewPlayerInfo:OnHandleEv(ev)
    ...
    elseif (ev.EventName == "EvEntityGetPlayerModuleDataSuccess") then
        ...
        if (player_moduledata_texas.GameTotal == 0) then
    ...
    elseif (ev.EventName == "EvBindWeChatSuccess") then
...
function ViewPlayerInfo:changeNickName()
    if (self.ChangeNameNeedItemTbId ~= nil and self.ChangeNameNeedItemTbId ~= "") then
        local changename_itemtbid = 0
        local n = tonumber(self.ChangeNameNeedItemTbId)
        if (n) then
            changename_itemtbid = n
        ...
        if (nick_name == nil or nick_name == "") then
        ...
        local ev = self:GetEv("EvUiClickChangePlayerNickName")
        if (ev == nil) then
...
function ViewPlayerInfo:changeIndividualSignature()
    ViewHelper:UiBeginWaiting(self.ViewMgr.LanMgr:getLanValue("ChangeInfo"))
    local ev = self:GetEv("EvUiClickChangePlayerIndividualSignature")
    if (ev == nil) then
...
function ViewPlayerInfo:onClickResetAddress()
    local ev = self:GetEv("EvUiClickRefreshIPAddress")
    if (ev == nil) then
...
function ViewPlayerInfo:onClickBindWeChat()
    if self.NeedBindWeChat then
        ...
        local ev = self:GetEv("EvUnbindWechat")
        if (ev == nil) then
...
---------------------------------------
ViewPlayerInfoFactory = class(ViewFactory)
