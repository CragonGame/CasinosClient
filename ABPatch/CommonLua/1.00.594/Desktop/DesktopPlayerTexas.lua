...
function DesktopPlayerTexas:onUpdate(elapsed_tm)
    ...
    then
        ...
        then
            ...
            --local check_stack_ifenough_minchip = self.DesktopTexas.DesktopTypeBase.SmallBlind
            --local check_ifchipenough_minchip = self.DesktopTexas.DesktopTypeBase.BetMin
            --if (self.DesktopTexas.DesktopConfigData.desktop_typetexas == DesktopTypeTexas.Classic)
            --then
            --    check_stack_ifenough_minchip = self.DesktopTexas.DesktopConfigData.small_blind
            --    check_ifchipenough_minchip = self.DesktopTexas.DesktopConfigData.bet_min
            --elseif (self.DesktopTexas.DesktopConfigData.desktop_typetexas == DesktopTypeTexas.MustBet)
            --then
            --    local tb_datamgr = TbDataMgr:new(nil)
            --    local tb_mustbet_desktopinfo = tb_datamgr:GetData("MustBetDesktopInfoTexas", self.DesktopTexas.DesktopConfigData.desktop_tbid)
            --    check_stack_ifenough_minchip = tb_mustbet_desktopinfo.MustBetChips
            --    check_ifchipenough_minchip = tb_mustbet_desktopinfo.CanPlayMinChips
            --end
