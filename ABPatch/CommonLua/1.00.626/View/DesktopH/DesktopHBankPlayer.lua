...
function DesktopHBankPlayer:setBankPlayerInfo(bankplayer_datadesktoph)
    ...
    if (self.BankPlayerDataDesktopH == nil or self.BankPlayerDataDesktopH.PlayerInfoCommon.PlayerGuid
            ~= bankplayer_datadesktoph.PlayerInfoCommon.PlayerGuid)
    then
...
function DesktopHBankPlayer:showCardsEnd()
    if (self.FTaskerShowCardEnd ~= nil)
    then
...
function DesktopHBankPlayer:showGameEndGoldAni()
    if (self.WinRewardPotGolds ~= nil and self.WinRewardPotGolds > 0)
    then
        if (self.FTaskerGetRewardGold ~= nil)
        then
...
function DesktopHBankPlayer:giveGoldToPot(pot_index)
    ...
    for k,v in pairs(self.ViewDesktopH:getDesktopHChairAll()) do
        ...
        then
        elseif (v.SeatPlayerInfoHundred.PlayerInfoCommon.PlayerGuid == self.ViewDesktopH.ControllerDesktopH.Guid)
        then
        else
...
function DesktopHBankPlayer:sendMagicExp(sender_guid, exp_tbid)
    ...
    local from_pos = nil
    if (self.BankPlayerDataDesktopH.PlayerInfoCommon.PlayerGuid == sender_guid)
    then
...
function DesktopHBankPlayer:_setBankPlayerGolds()
    ...
    if (CS.Casinos.CasinosContext.Instance.DesktopHSysBankShowDBValue and
            CS.System.String.IsNullOrEmpty(self.BankPlayerDataDesktopH.PlayerInfoCommon.PlayerGuid))
    then
...
function DesktopHBankPlayer:_onClick()
    if (CS.System.String.IsNullOrEmpty(self.BankPlayerDataDesktopH.PlayerInfoCommon.PlayerGuid) == false)
    then
        local view_mgr = ViewMgr:new(nil)
