...
function ViewGiftDetail:setGift(gift_id,is_buygift,is_mine,to_etguid,from_name,gift_belong,item)
	...
	local title = self.ViewMgr.LanMgr:getLanValue("移除物品", "DeleteGoods")
	local confirm_text = self.ViewMgr.LanMgr:getLanValue("移除", "Delete")
	...
	if(CS.Casinos.PriceType.__CastFrom(tb_gift.PriceType) == CS.Casinos.PriceType.Gold)
	then
		buy_price = buy_price .. self.ViewMgr.LanMgr:getLanValue("钻石", "Diamonds")
		sell_price = sell_price .. self.ViewMgr.LanMgr:getLanValue("钻石", "Diamonds")
		all_buyprice = all_buyprice .. self.ViewMgr.LanMgr:getLanValue("钻石", "Diamonds")
	end
	...
	if (self.mBuyGift)
	then
		title = self.ViewMgr.LanMgr:getLanValue("购买临时礼物", "BuyTempGift")
		confirm_text = self.ViewMgr.LanMgr:getLanValue("购买", "Buy") .. ":" .. buy_price
		buy_all = self.ViewMgr.LanMgr:getLanValue("给所有人买", "BuyEveryone") .. ":" .. all_buyprice
	else
		...
		then
			...
			then
				...
				confirm_text = self.ViewMgr.LanMgr:getLanValue("出售", "Sell") .. ":" .. sell_price
				self.mSellGift = true
			else
				title = self.ViewMgr.LanMgr:getLanValue("物品", "Goods")
				other_playergift = true
			end
			gift_detail = gift_detail .. ("\n" .. self.ViewMgr.LanMgr:getLanValue("来自于", "From") .. from_name)
		end
	end
	if(tb_gift.UnitType == "GoodsVoucher")
	then
		title = self.ViewMgr.LanMgr:getLanValue("兑换券","兑换券")
		gift_detail = self.ViewMgr.LanMgr:getLanValue("兑换券可以用来兑换实物","兑换券可以用来兑换实物")
		confirm_text = self.ViewMgr.LanMgr:getLanValue("兑换","兑换")
