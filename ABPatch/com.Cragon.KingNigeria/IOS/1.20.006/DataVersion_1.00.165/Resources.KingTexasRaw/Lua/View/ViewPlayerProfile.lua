...
function ViewPlayerProfile:onCreate()
    ...
    self.UiHeadIcon = ViewHeadIcon:new(nil,co_headicon,
		function()
			ViewHelper:UiBeginWaiting(self.ViewMgr.LanMgr:getLanValue("获取大图中...", "GetBigPic"))
				...
				CS.Casinos.HeadIconMgr.Instant:asyncLoadIcon(icon_name .. "_Big",CS.Casinos.HeadIconMgr.getIconURL(false, icon_name),icon_name, nil,
					function(ex,tick)
						...
						then
							...
							local texture = CS.Casinos.LuaHelper.UnityObjectCastToTexture(ex)
...
function ViewPlayerProfile:sendMagicExp(exp_tbid)
	...
	then
		ViewHelper:UiShowInfoFailed(self.ViewMgr.LanMgr:getLanValue("不可以送自己魔法表情!", "NotGiveSelfMagic"))
		return
	end
	...
	then
        ...
        msg_box:showMsgBox1(self.ViewMgr.LanMgr:getLanValue("不是玩家", "NotPlayer"),self.ViewMgr.LanMgr:getLanValue("你必须坐在牌桌旁才能发送魔法表情", "SitTableSendMagic"),nil)
        return
	end
...
function ViewPlayerProfile:setPlayerInfo(player_info)
    ...
    self.GTextPlayerNickName.text = self.PlayerInfo.PlayerInfoCommon.NickName
    ...
    local address = self.ViewMgr.LanMgr:getLanValue("地址", "Address")
    local real_address = self.PlayerInfo.PlayerInfoMore.IPAddress
    if (real_address == nil or real_address == "")
	then
		real_address = self.ViewMgr.LanMgr:getLanValue("未知", "Unknown")
	end
    ...
    self.ListGift = {}
	for i = 0,self.PlayerInfo.PlayerInfoMore.ListItemData.Count - 1 do
		local item = Item:new(nil,self.ViewMgr.TbDataMgr,self.PlayerInfo.PlayerInfoMore.ListItemData[i])
        if (item.UnitLink.UnitType ~= CS.Casinos.UnitType.GiftNormal)
		then
		else
    ...
    local btn_addfriend_title = self.ViewMgr.LanMgr:getLanValue("添加好友", "AddFriend")
    if (is_friend)
	then
		btn_addfriend_title = self.ViewMgr.LanMgr:getLanValue("删除好友", "DeleteFriend1")
	end
		...
		function(x,y) 
			...
			then
				...
				then
					...
					local gift_normalx = CS.Casinos.LuaHelper.UnitCastToUnitGiftNormal(x.UnitLink)
					local gift_normaly = CS.Casinos.LuaHelper.UnitCastToUnitGiftNormal(y.UnitLink)
...
function ViewPlayerProfile:RenderListItem(index,obj)
	...
	then
        ...
        if (item.UnitLink.UnitType == "UnitGiftNormal")
		then
