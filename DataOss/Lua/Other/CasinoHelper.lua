...
function CasinoHelper:TranslateFriendStateEx(friend_info)
    ...
    if (online_state == PlayerOnlineState.Offline) then
        friend_state = _eFriendStateClient.Offline
    elseif (online_state == PlayerOnlineState.Online) then
        local player_playstate = friend_info.PlayerPlayState
        if (player_playstate ~= nil and CS.System.String.IsNullOrEmpty(player_playstate.DesktopGuid) == false) then
            local module = player_playstate.CasinosModule
            if (module == CasinosModule.Fishing) then
                friend_state = _eFriendStateClient.Fishing
            elseif (module == CasinosModule.GFlower) then
                if (player_playstate.DesktopType == DesktopTypeEx.Desktop) then
            ...
            elseif (module == CasinosModule.Texas) then
                if (player_playstate.DesktopType == DesktopTypeEx.Desktop) then
                    if (CS.System.String.IsNullOrEmpty(player_playstate.UserData2) == false) then
            ...
            elseif (module == CasinosModule.NiuNiu) then
...
function CasinoHelper:TranslateFriendState(friend_state)
    ...
    if (friend_state == _eFriendStateClient.Offline) then
        state = lan_mgr:getLanValue("OffLine")
    elseif (friend_state == _eFriendStateClient.Fishing) then
        state = lan_mgr:getLanValue("Fishing")
    elseif (friend_state == _eFriendStateClient.GFlowerDesktopH) then
        state = lan_mgr:getLanValue("ZhaJinHuaDesktopH")
    elseif (friend_state == _eFriendStateClient.GFlowerDesktopPrivate) then
        state = lan_mgr:getLanValue("ZhaJinHuaUnknow")
    elseif (friend_state == _eFriendStateClient.GFlowerDesktopNormal) then
        state = lan_mgr:getLanValue("ZhaJinHuaClassic")
    elseif (friend_state == _eFriendStateClient.TexasDesktopClassicPrivate) then
        state = lan_mgr:getLanValue("TexasPokerUnknow")
    elseif (friend_state == _eFriendStateClient.TexasDesktopClassic) then
        state = lan_mgr:getLanValue("TexasPokerClassic")
    elseif (friend_state == _eFriendStateClient.TexasDesktopMustBet) then
        state = lan_mgr:getLanValue("TexasPokerMustBet")
    elseif (friend_state == _eFriendStateClient.TexasDesktopMustBetPrivate) then
        state = lan_mgr:getLanValue("TexasPokerUnknow")
    elseif (friend_state == _eFriendStateClient.TexasDesktopH) then
        state = lan_mgr:getLanValue("TexasPokerDesktopH")
    elseif (friend_state == _eFriendStateClient.NiuNiuDesktopH) then
        state = lan_mgr:getLanValue("NiuNiuDesktopH")
    elseif (friend_state == _eFriendStateClient.ZhongFBDesktopH) then
        state = lan_mgr:getLanValue("ZhongFaBaiDesktopH")
    elseif (friend_state == _eFriendStateClient.NotInDesktop) then
        state = lan_mgr:getLanValue("InLobby")
    elseif (friend_state == _eFriendStateClient.DesktopMatch) then
