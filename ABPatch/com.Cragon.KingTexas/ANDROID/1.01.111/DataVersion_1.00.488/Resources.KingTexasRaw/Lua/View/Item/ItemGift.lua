...
function ItemGift:setGift(gift_id,is_buygift,is_mine,to_etguid,from_name,gift_belong,item)
	...
	then
		...
		if self.GTextItemDes ~= nil then
			if item.UnitLink.UnitType == "RedEnvelopes" then
				self.GTextItemDes.text = tostring(item.UnitLink.Value) .. self.LanMgr:getLanValue("Yuan")
			end
		end
	end
