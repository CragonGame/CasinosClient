...
function ViewFriend:onCreate()
    ViewHelper:PopUi(self.ComUi,"好友")
    ...
    self.GTextInputSerch.promptText = self.CasinosContext:parseLanStr("昵称或ID", "SearchPlayerTips")
    self.GTextID = self.ComUi:GetChild("TextID").asTextField
    self.GBtnSearch = self.ComUi:GetChild("BtnSearch").asButton
    ...
    self.GTextID.text = self.CasinosContext:parseLanStr("我的", "My") .. "ID:" .. str_id_1 .. "-" .. str_id_2 .. "-" .. str_id_3
...
function ViewFriend:setRecommandFriend(list_friend)
    ...
    if (#list_friend == 0)
    then
        ViewHelper:UiShowInfoFailed(self.CasinosContext:parseLanStr("未搜索到好友", "NotSearchFriend"))
...
function ViewShowFriendDetail:setFriendInfo(friend_item)
    ...
    then
        address = self.CasinosContext:parseLanStr("未知", "Unknown")
    end
    address = self.CasinosContext:parseLanStr("地址：", "Address") .. address
    ...
    if (friend_state == CS.Casinos._eFriendStateClient.TexasDesktopClassic or
            friend_state == CS.Casinos._eFriendStateClient.TexasDesktopMustBet or
            friend_state == CS.Casinos._eFriendStateClient.GFlowerDesktopNormal)
    then
    ...
    if (self.IsFriendDetail)
    then
        if (friend_item.PlayerInfoMore.OnlineState == CS.Casinos.PlayerOnlineState.Online)
        then
    ...
    local btn_addfriend_title = self.CasinosContext:parseLanStr("添加好友", "AddFriend")
    if (is_friend)
    then
        btn_addfriend_title = self.CasinosContext:parseLanStr("删除好友", "DeleteFriend1")
    end
...
function ViewShowFriendDetail:onClickCommonHeadIcon()
    ViewHelper:UiBeginWaiting(self.CasinosContext:parseLanStr("获取大图中…", "GetBigPic"))
        ...
        CS.Casinos.HeadIconMgr.Instant:asyncLoadIcon(icon_name .. "_Big", CS.Casinos.HeadIconMgr.getIconURL(false, icon_name), icon_name, nil,
                function(ex, tick)
                    ...
                    then
                        ...
                        ex = CS.Casinos.LuaHelper.UnityObjectCastToTexture(ex)
...
function ViewShowFriendDetail:onClickBtnJoin()
    ...
    if (self.CurrentFriendInfo.PlayerInfoMore.OnlineState == CS.Casinos.PlayerOnlineState.Offline)
    then
        return
    elseif (self.CurrentFriendInfo.PlayerInfoMore.OnlineState == CS.Casinos.PlayerOnlineState.Online)
    then
        if (self.CurrentFriendInfo.PlayerPlayState == nil)
        then
            return
        else
            if (self.CurrentFriendInfo.PlayerPlayState.DesktopType == CS.Casinos.DesktopType.Desktop)
            then
                ...
                then
                    ...
                    ViewHelper:UiShowInfoFailed(self.CasinosContext:parseLanStr("该玩家不在桌内!", "PlayerNotInTable"))
                end
            elseif (self.CurrentFriendInfo.PlayerPlayState.DesktopType == CS.Casinos.DesktopType.DesktopH)
            then
