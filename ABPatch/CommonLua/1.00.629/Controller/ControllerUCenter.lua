...
function ControllerUCenter:nigWebpayRequestUrl(handler)
    ...
    local http_url = self:_genPayUrl("nigeriawebpay")

    self.WWWNigWebpayRequestUrl = CS.UnityEngine.WWW(http_url)
end
...
function ControllerUCenter:nigWebpayQuery(handler)
    ...
    local http_url = self:_genPayUrl("nigeriawebpayquery")

    self.WWWNigWebpayQuery = CS.UnityEngine.WWW(http_url)
end

function ControllerUCenter:quicktellerTransfers(handler)
    if (self.WWWQuicktellerTransfers ~= nil)
    then
        return
    end
    ...
    local http_url = self:_genPayUrl("paynigeria/quickteller/payments/transfers")

    self.WWWQuicktellerTransfers = CS.UnityEngine.WWW(http_url)
end
...
function ControllerUCenter:quicktellerTransfersQuery(handler)
    ...
    local http_url = self:_genPayUrl("paynigeria/quickteller/payments/transfersquery")

    self.WWWQuicktellerTransfersQuery = CS.UnityEngine.WWW(http_url)
end
