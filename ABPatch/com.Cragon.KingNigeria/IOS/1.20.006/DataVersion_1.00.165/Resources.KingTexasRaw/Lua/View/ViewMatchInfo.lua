...
function ViewMatchInfo:onCreate()
    ...
    ViewHelper:PopUi(self.ComUi,"本场赛况")
    ...
    self.GTextBtnApplyTitle = self.GBtnApply:GetChild("Title").asTextField
    ...
    self.GTextRankTips = com_rank:GetChild("TextRankTips").asTextField
...
function ViewMatchInfo:onUpdate(tm)
    ...
    then
        local matchUseTime = CS.System.DateTime.Now - self.DtMatchBegin
...
function ViewMatchInfo:SetMatchInfo(detailedMatchInfo)
    ...
    then
        ...
        text_signUpfee = UiChipShowHelper:getGoldShowStr3(match_info.SignupFee) .. "+" .. UiChipShowHelper:getGoldShowStr3(match_info.ServiceFee) .. "(参赛费+服务费)"
    end
    ...
    self.GTextEntryNumber.text = tostring(match_info.PlayerNum)
    ...
    then
        ...
        local temp = {}
        temp[1] = "可重购买比赛,次数: [color=#DDBB0E]"
        temp[2] = tostring(match_info.CanRebuyCount)
        temp[3] = "[/color]次\n第[color=#DDBB0E]"
        if (#blind_list == 1)
        then
            local level = blind_list[1] - raiseBlindInfo.BeginId + 1
            temp[4] = tostring(level)
        elseif (#blind_list > 1)
        then
            local start_level = blind_list[1] - raiseBlindInfo.BeginId + 1
            local end_level = blind_list[#blind_list] - raiseBlindInfo.BeginId + 1
            temp[4] = tostring(start_level) .. "-" .. tostring(end_level)
        end
        temp[5] = "[/color]个盲注级别可用[color=#DDBB0E]"
        temp[6] = UiChipShowHelper:getGoldShowStr3(raiseBlindInfo.RebuyGold)
        temp[7] = "[/color]兑换[color=#DDBB0E]"
        temp[8] = UiChipShowHelper:getGoldShowStr3(raiseBlindInfo.RebuyScore)
        temp[9] = "[/color]筹码"
        self.GComReBuy.visible = true
        self.GTextReBuyTips.text = table.concat(temp)
    else
    ...
    then
        ...
        local temp = {}
        temp[1] = "可增购买比赛,次数: [color=#DDBB0E]"
        temp[2] = tostring(match_info.CanAddonCount)
        temp[3] = "[/color]次\n第[color=#DDBB0E]"
        if (#addon_list == 1)
        then
        ...
        temp[5] = "[/color]个盲注级别可用[color=#DDBB0E]"
        temp[6] = UiChipShowHelper:getGoldShowStr3(raiseBlindInfo.AddonGold)
        temp[7] = "[/color]兑换[color=#DDBB0E]"
        temp[8] = UiChipShowHelper:getGoldShowStr3(raiseBlindInfo.AddonScore)
        temp[9] = "[/color]筹码"
        self.GGroupAddon.visible = true
        self.GTextAddonTips.text = table.concat(temp)
    else
    ...
    for i = raiseBlindInfo.BeginId, raiseBlindInfo.EndId do
        ...
        ItemMatchInfoBlind:new(nil, com, data, blind_level, canRebuy, canAddon, raiseBlindInfo.RaiseBlindTmSpan)
	...
	then
		...
		local temp = {}
		temp[1] = "保底奖励价值:[color=#CFB37A]"
		temp[2] = UiChipShowHelper:getGoldShowStr3(reward_info.SnowballTotalReward)
		temp[3] = "德州币"
		tips = table.concat(temp)
	else
		self.GComRewardExplain.visible = false
		tips = match_info.Name .. tabletips
	end
    self.GTextRewardTips.text = tips
	if(#reward_info.ListReward > 0)
	then
		for  i = 1,#reward_info.ListReward do
			local reward = reward_info.ListReward[i]
			local com = self.GListReward:AddItemFromPool()
			ItemMatchInfoReward:new(nil,com,reward)
			if(i == #reward_info.ListReward and isSnowBallReward)
			then
				...
				aTextField.text = "[size=22][color=#CFB37A]参赛人数越多，奖励人数越多，总奖励越高![/color][/size]"
			end
		end
	end
end
...
function ViewMatchInfo:onClickShare()
    local content = string.format("%s邀请你参加%s的%s，赶快报名，一起战斗！",self.ControllerActor.PropNickName:get(),
    ...
    Native.Instance:ShareContent(CS.cn.sharesdk.unity3d.PlatformType.WeChat, content, pic_path, self.MatchInfo.Name.."赛事邀请",
...
function ViewMatchInfo:onClickBtnApply()
    ...
    then
        ...
        local content = "退赛将退还全部报名费(门票)和服务费。您确定退赛吗？"
...
function ViewMatchInfo:getBtnApplyState()
    ...
    then
        ...
        then
            ...
            self.GTextBtnApplyTitle.text = "退赛"
            return 3 -- 已经报名但不可退赛
        elseif (leftMatchBeginTime.Minutes <= 0)
        then
            self.GTextBtnApplyTitle.text = "进入"
            return 2 -- 已经报名 可以进入
        else
            self.GTextBtnApplyTitle.text = "退赛"
            return 3 -- 已经报名 可以退赛
        ...
        self.GTextBtnApplyTitle.text = "报名"
        return 4 -- 尚未报名
    end
end
...
function ViewMatchInfo:formatTime(dtTime)
    ...
    then
        ...
        local temp = {}
        temp[1] = string.format("%02s", dtTime.Month)
        temp[2] = "月"
        temp[3] = tostring(dtTime.Day)
        temp[4] = "日"
        text_day = table.concat(temp)
    end
    local temp = {}
