...
function ViewQuitOrBack:onCreate()
    ...
    self.BtnQuit = self.ComUi:GetChild("Lan_Btn_Quit").asButton
...
function ViewQuitOrBack:setQuitTimeAndIfCanBackGameAndTips(quit_tm,can_back,tips)	
	...
	if(final_tips == nil or final_tips == "")
	then
		final_tips = CS.Casinos.CasinosContext.Instance:GetRandomTips()
	end
...
function ViewQuitOrBack:setCountDownTm()
	local tips = self.ViewMgr.LanMgr:getLanValue("退出游戏?" .. "QuitGame")
...
function ViewQuitOrBackFactory:new(o,ui_package_name,ui_component_name,
