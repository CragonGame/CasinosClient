...
function ItemInviteFriendPlay:setFriendInfo(friend_item)
    ...
    self.GTextName.text = friend_item.PlayerInfoCommon.NickName
...
function ItemInviteFriendPlay:onClickItem()
	...
	then
        ...
        view_mgr:sendEv(ev)
        self.GTextTm.text = view_mgr.LanMgr:getLanValue("已邀请", "HaveInvited")
        self.GTextTm.color = CS.UnityEngine.Color.yellow
        self.AlreadyInvite = true
	else
		ViewHelper:UiShowInfoFailed(view_mgr.LanMgr:getLanValue("已邀请，请勿重复邀请!", "DontRepeatInvitation"))
	end
end
