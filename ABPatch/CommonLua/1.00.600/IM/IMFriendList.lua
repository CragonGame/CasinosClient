...
function IMFriendList:OnIMFriendLoginNotify(player_guid)
	...
	then
		...
		if(#self.ListOffLineFriendGuid > 0)
		then
			for i = 1,#self.ListOffLineFriendGuid do
				if(player_guid == self.ListOffLineFriendGuid[i])
				then
		...
		if(#self.ListOnLineFriendGuid > 0)
		then
			for i = 1,#self.ListOnLineFriendGuid do
				if(player_guid == self.ListOnLineFriendGuid[i])
				then
					contains = true
					break
				end
...
function IMFriendList:OnIMFriendListAddNotify(list_player)
	for key,value in pairs(list_player)do
		...
		if (p_i.PlayerInfoMore.OnlineState == PlayerOnlineState.Offline)
		then
...
function IMFriendList:OnIMFriendListRemoveNotify(player_guid)
	...
	if(self.MapFriendList[player_guid] ~= nil)
	then
...
function IMFriendList:OnIMEventPush2ClientNotify(ev)
	...
	i_e:setData(ev)
	if(i_e.EvId == IMOfflineEventId.AddFriendRequest)
	then
...
function IMFriendList:getFriendRecordIsChecked(friend_guid)
	...
	if(self.MapFriendRecordIsChecked[friend_guid] ~= nil)
	then
...
function IMFriendList:getFriendState(friend_guid)
	...
	if(self.MapFriendState[friend_guid] ~= nil)
	then
