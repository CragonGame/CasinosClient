...
public class RpcSessionTcpClient : RpcSession
{
    ...
    void _onSocketReceive(byte[] data, int len)
    {
        ...
        if (method_id >= Casinos.CasinosContext.Instance.CSharpLastMethodId)
        {
            ...
            //    var desktopinfo = MessagePackSerializer.Deserialize<List<DesktopInfoTest>>(buf);
            //    foreach (var i in desktopinfo)
            //    {                    
            //        try
            //        {
            //            var d = MessagePackSerializer.Deserialize<DesktopInfoTexasTest>(i.DesktopData);
            //            string ss = "";
            //        }
            //        catch (Exception ex)
            //        {
