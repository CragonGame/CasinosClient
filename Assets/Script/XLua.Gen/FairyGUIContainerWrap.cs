    ...
    public class FairyGUIContainerWrap 
    {
        ...
        static int _m_HitTest(RealStatePtr L)
        {
		    try {
                ...
                if(gen_param_count == 4&& translator.Assignable<UnityEngine.Vector2>(L, 2)&& LuaTypes.LUA_TBOOLEAN == LuaAPI.lua_type(L, 3)&& LuaTypes.LUA_TNUMBER == LuaAPI.lua_type(L, 4)) 
                {
                    UnityEngine.Vector2 _stagePoint;translator.Get(L, 2, out _stagePoint);
                    bool _forTouch = LuaAPI.lua_toboolean(L, 3);
                    int _displayIndex = LuaAPI.xlua_tointeger(L, 4);
                    
                        FairyGUI.DisplayObject gen_ret = gen_to_be_invoked.HitTest( _stagePoint, _forTouch, _displayIndex );
                        translator.Push(L, gen_ret);
            ...
            return LuaAPI.luaL_error(L, "invalid arguments to FairyGUI.Container.HitTest!");
