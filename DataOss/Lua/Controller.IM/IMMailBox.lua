...
function IMMailBox:OnIMMailUpdateNotify(mail)
	...
	local current_mail = nil
	for key,value in pairs(self.ListMail) do
		if(value.MailGuid == m_c.MailGuid)
		then
			current_mail = value
		end
	end
...
function IMMailBox:OnIMMailOperateRequestResult(result)
	...
	m_r:setData(result)
	if (m_r.OperateType == MailOperateType.Read or m_r.OperateType == MailOperateType.Delete)
	then
		return
	end
	...
	if (m_r.Result == ProtocolResult.Success)
	then
		...
		local current_mail = nil
		for key,value in pairs(self.ListMail) do
			if(value.MailGuid == m_r.MailGuid)
			then
				current_mail = value
			end
		end
		if (current_mail ~= nil and current_mail.Attachment ~= nil and current_mail.MailState ~= MailState.ReadAndRecvAttachments)
		then
			sb:Append(self.ControllerIM.ControllerMgr.LanMgr:getLanValue("Get"))
			if (current_mail.SenderType == MailSenderType.Player)
			then
			...
			if (current_mail.Attachment.Gold > 0)
			then
			...
			if (current_mail.Attachment.Diamond > 0)
			then
			...
			if (current_mail.Attachment.ListItem ~= nil)
			then
		...
		if (m_r.Result == ProtocolResult.BagFull)
		then
...
function IMMailBox:getOperateTitle(operate_type)
	local title = ""
	if(operate_type == MailOperateType.None)
	then
	elseif(operate_type == MailOperateType.Read)
	then
		title = self.ControllerIM.ControllerMgr.LanMgr:getLanValue("ReadMail")
	elseif(operate_type == MailOperateType.RecvAttachment)
	then
		title = self.ControllerIM.ControllerMgr.LanMgr:getLanValue("TakeAttachment")
	elseif(operate_type == MailOperateType.Delete)
	then
...
function IMMailBox:updateNewMailSign()
	for key,value in pairs(self.ListMail) do
		if (value.MailState == MailState.New)
		then
			self.HaveNewMail = true
			break;
		else
