    ...
    then
        ...
        elseif (ev.EventName == "EvUiLoginSuccessEx") then
            if self.BindingWeChat == false then
                ...
                print("EvUiLoginSuccessEx1")
                local request = AccountWeChatBindRequest:new(nil)
                request.AppId = CS.Casinos.CasinosContext.Instance.Config.WeChatAppId
                print("EvUiLoginSuccessExAPPID  " .. CS.Casinos.CasinosContext.Instance.Config.WeChatAppId)
                request.Code = ev.token
                print("EvUiLoginSuccessCode   " .. ev.token)
                request.AccountId = self.AccId
                print("EvUiLoginSuccessAccId  " .. self.AccId)
                request.Token = self.Token
                print("EvUiLoginSuccessToken  ".. self.Token)
...
function ControllerLogin:onUCenterBindWeChat(status, response, error)
    print("onUCenterBindWeChat")
    if (status == UCenterResponseStatus.Success)
    then
        print("onUCenterBindWeChat1")
        if (response ~= nil and response.result == UCenterErrorCode.NoError)
        then
            print("onUCenterBindWeChat2")
        ...
        if (response ~= nil)
        then
            print("onUCenterBindWeChat3  " ..response.result)
            ViewHelper:UiEndWaiting()
            ViewHelper:UiShowInfoFailed(self.ControllerUCenter:ParseUCenterErrorCode(response.result))
        else
            ViewHelper:UiEndWaiting()
            ViewHelper:UiShowInfoFailed(self.ControllerUCenter:ParseUCenterErrorCode(error.code))
            print("onUCenterBindWeChat4")
        end
    end
end
