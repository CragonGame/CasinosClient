...
function ControllerTrade:onCreate()
    ...
    --rpc:RegRpcMethod1(m_c.TradeBuyRMBItemSuccessResponse, function(response)
    --    self:OnTradeBuyRMBItemSuccessResponse(response)
    --end)
...
function ControllerTrade:onHandleEv(ev)
    ...
    then
        self.ControllerMgr.Listener.UniWebView:Load("http://kingnigeria-ucenter.cragon.cn:81/nigeriawebpay")
        ...
        --end)
    elseif (ev.EventName == "EvUiRequestQuicktellerTransfers")
    then
...
--function ControllerTrade:OnTradeBuyRMBItemSuccessResponse(response1)
--    local response = BuyRMBItemSuccessResponse:new(nil)
--    response:setData(response1)
--    if (response.result == ProtocolResult.Success)
--    then
