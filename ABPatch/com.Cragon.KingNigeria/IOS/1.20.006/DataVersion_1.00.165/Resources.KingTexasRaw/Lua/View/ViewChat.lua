...
function ViewChat:onDestroy()
	if(self.GTextInputChat.text ~= nil and #self.GTextInputChat.text > 0)
	then
...
function ViewChat:onHandleEv(ev)
	if(ev.EventName == "EvEntityRecvChatFromDesktopH")
	then
		self.MapChat = self.ControllerPlayer:getDesktopChat()
...
function ViewChat:init(chat_type)
	...
	then
        ...
        if (CS.UnityEngine.PlayerPrefs.HasKey(self.PlayTanMuKey))
		then
			local return_value,use_tanmu = CS.System.Boolean.TryParse(CS.UnityEngine.PlayerPrefs.GetString(self.PlayTanMuKey))
		end
    ...
    local btn_send = self.ComUi:GetChildInGroup(group_parent,"BtnSend").asButton
...
function ViewChat:RenderListItem(index,obj)
    ...
    local chat_info
	if(self.MapChat[index] ~= nil)
	then
		chat_info = self.MapChat[index]
		if(chat_info.sender_etguid == "")
		then
			item:setChat(chat_info.sender_name, chat_info.chat_content, chat_info.sender_viplevel,
                         CS.System.DateTime.MinValue, "FFFF00", "FFFFFF", true, self.GListChat.width - 50, CS.Casinos._eChatItemType.NormalChat,
                         true, true, true, false, false)
		else
			item:setChat(chat_info.sender_name, chat_info.chat_content, chat_info.sender_viplevel,
                         CS.System.DateTime.MinValue, "", "FFFFFF", true, self.GListChat.width - 50, CS.Casinos._eChatItemType.NormalChat, true, true)
		end
	end
end
...
function ViewChat:setPresetMsg()
	...
	do
		local msg = value
        if (self.UiChatType == CS.Casinos._eUiChatType.Desktop and CS.Casinos.PresetMsgType.__CastFrom(msg.PresetMsgType) == CS.Casinos.PresetMsgType.DesktopH)
		then
        elseif (self.UiChatType == CS.Casinos._eUiChatType.DesktopH and CS.Casinos.PresetMsgType.__CastFrom(msg.PresetMsgType) == CS.Casinos.PresetMsgType.Desktop)
		then
		else
			local co_presetmsg = self.GListShortCutChat:AddItemFromPool().asCom
			local msg_content = self.ViewMgr.LanMgr:getLanValue(msg.PresetMsg, msg.PresetMsg)
...
function ViewChat:onClickSendChat()
	if(self.GTextInputChat.text == nil or self.GTextInputChat.text == "")
	then
		ViewHelper:UiShowInfoFailed(self.ViewMgr.LanMgr:getLanValue("�������ݲ���Ϊ��!", "SendNoEmpty"))
        return
	end
    local can_chat = false
    if (self.UiChatType == CS.Casinos._eUiChatType.Desktop)
	then
		if (self.ControllerActor.PropVIPLevel:get() >= self.CasinosContext.DesktopCanChatVIPLimit)
		then
			can_chat = true
		end
	elseif(self.UiChatType == CS.Casinos._eUiChatType.DesktopH)
	then
		if (self.ControllerActor.PropVIPLevel:get() >= self.CasinosContext.DesktopHCanChatVIPLimit)
		then
			can_chat = true
		else
			ViewHelper:UiShowInfoSuccess(self.ViewMgr.LanMgr:getLanValue("VIP玩家才可发送消息", "OnlyVIPCanSend"))
		end
	end
