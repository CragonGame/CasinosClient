...
function ViewMain:onCreate()
	...
	CS.Casinos.UiHelperCasinos.SetParticle(gold_par_parent, "Particle/GoldTreePar")
	local com_more = nil
	...
	local co_onlinereward = self.ComUi:GetChild("ComOnlineReward").asCom
	self.ViewOnlineReward = ViewOnlineReward:new(nil,co_onlinereward,self.ViewMgr)
	self.ViewOnlineReward:setCanGetReward(self.ControllerPlayer.OnLineReward:IfCanGetReward())
	...
	then
		...
		--local holder_mote = self.ComUi:GetChild("HolderMote").asGraph
		...
		--holder_mote:SetNativeObject(CS.FairyGUI.GoWrapper(self.PlayerAnim.transform.gameObject))
	else
...
function ViewMain:onHandleEv(ev)
	...
	then
		...
		then
			self.GTextLotteryTicketTips.text = self.CasinosContext:parseLanStr("结算中", "Settlement")
...
function ViewMain:updateLotteryTickTm(tm)
	if (tm > 0)
	then
		self.GTextLotteryTicketTips.text = tm .. self.CasinosContext:parseLanStr("秒", "S")
	else
		self.GTextLotteryTicketTips.text = self.CasinosContext:parseLanStr("结算中", "Settlement")
	end
end
...
function ViewMain:setOnlineNum(num)
	local online_player = "在线玩家"
	if (self.CasinosContext.UseLan)
	then
		online_player = self.ViewMgr.LanMgr:getLanValue("在线玩家","OnlinePlayer")
	end
	...
	table.sort(self.ListFriendInfo,
			function(f1,f2)
				...
				then
					...
					if(f1.PlayerInfoMore.OnlineState == CS.Casinos.PlayerOnlineState.Online)
					then
...
function ViewMain:setCurrentFriendInfo(friend_item)
	...
	self.UiPlayerInfoCurrentFriend:setPlayerInfo(self.CurrentFriendItem.PlayerInfoCommon.NickName,
			...
			self.CurrentFriendItem.PlayerInfoMore.OnlineState == CS.Casinos.PlayerOnlineState.Online)
end

function ViewMain:setLotteryTicketInfo(state,left_tm)
	local tips = ""
	if (state == CS.Casinos.LotteryTicketStateEnum.Bet)
	then
		tips = math.ceil(left_tm) .. self.CasinosContext:parseLanStr("秒", "S")
	else
		tips = self.CasinosContext:parseLanStr("结算中", "Settlement")
	end
...
function ViewMain:refreshCurrentFriendInfo(player_info)
	...
	then
		...
		self.UiPlayerInfoCurrentFriend:setPlayerInfo1(player_info.PlayerInfoCommon.NickName,
				...
				friend_state,
				player_info.PlayerInfoMore.OnlineState == CS.Casinos.PlayerOnlineState.Online)
	end
end
