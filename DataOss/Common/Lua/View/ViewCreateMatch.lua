...
function ViewCreateMatch:onClickBtnReturn()
    if (self.GControllerSetType.selectedIndex == 1) then
...
function ViewCreateMatch:onClickBtnTabSet()
    if (self.GControllerSetType.selectedIndex == 0) then
...
function ViewCreateMatch:onSliderStartScoreChanged()
    ...
    if (value ~= nil) then
        ...
        for i = 1, #list_data do
            ...
            if (temp == list_data[i].BlindsBig) then
                break
            elseif (list_data[i].BlindsBig > temp) then
                temp1[i] = nil
                break
            end
        end
...
function ViewCreateMatch:onClickBtnCreate()
    ...
    if (self.MatchBeginTime < nowTm) then
...
function ViewCreateMatch:onClickBtnEnsureTime()
    ...
    if (result == false) then
        ...
        if (match_begintime < now_time) then
            ViewHelper:UiShowInfoSuccess("请输入正确的时间")
            return
        end
...
function ViewCreateMatch:DataIsLegal(month, day, hour, minute)
    if (month == nil or day == nil or hour == nil or minute == nil) then
        return false
    end
    if (month <= 0 or day <= 0 or hour < 0 or minute < 0) then
        return false
    end
    if (month > 12) then
        return false
