...
function ViewHeadIcon:setGift(gift_tbid)
	...
	then
        ...
        self.GLoaderCurrentGift.icon = CS.Casinos.CasinosContext.Instance.PathMgr:combinePersistentDataPath
...
function ViewHeadIcon:setPlayerIcon(icon, account_id)
	...
	then
		...
		then
			...
			if(icon ~= nil and string.len(icon) > 0)
			then
                self.GLoaderPlayerIcon.icon = CS.Casinos.CasinosContext.Instance.UserConfig.Current.PlayerIconDomain .. icon
			end
		end
	end
end
