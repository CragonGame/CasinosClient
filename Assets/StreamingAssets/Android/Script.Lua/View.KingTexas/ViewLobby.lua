...
function ViewLobby:onCreate()
	...
	self.GListBetRangeOperate.scrollPane.onScrollEnd:Add(
			function()
				self:onBetChipOperateListScrollEnd()-- 滚动停止
			end
	)
	...
	self.GListFriendHeadIcon.itemRenderer = function(index,item)
		self:rendererPlayingFriend(index,item)
	end
...
function ViewLobby:onHandleEv(ev)
	...
	then
		...
		elseif(ev.EventName == "EvEntitySearchDesktopFollowFriend")-- 一次只能看一个好友的所在桌，下方好友列表有选中态
		then
		...
		elseif(ev.EventName == "EvEntityFriendOnlineStateChange")-- 下方单个好友在线状态刷新，变在线不管，变离线删除
		then
			if (self.ControllerFriendOrBet.selectedIndex == 0)
			then
		...
		elseif(ev.EventName == "EvEntityNotifyDeleteFriend")-- 下方好友列表响应删好友消息
		then
			if (self.ControllerFriendOrBet.selectedIndex == 0)
			then
		...
		elseif(ev.EventName == "EvEntityRefreshFriendList")-- 下方好友列表响应好友列表刷新消息
		then
			if (self.ControllerFriendOrBet.selectedIndex == 0)
			then
				self:refreshPlayingFriend()
			end
		elseif(ev.EventName == "EvEntityRefreshFriendInfo")-- 刷新单个好友信息，比如头像昵称
		then
			if (self.ControllerFriendOrBet.selectedIndex == 0)
			then
				self:refreshPlayingFriend(ev.player_info)
			end
		elseif(ev.EventName == "EvEntitySearchPlayingFriend")-- 初始创建正在桌内玩的好友列表
		then
			self:setCurrentPlayingFriend(ev.list_playerinfo)
		elseif(ev.EventName == "EvClickIconWithNickName")-- 切换正在桌内玩的好友列表中的Item
		then
