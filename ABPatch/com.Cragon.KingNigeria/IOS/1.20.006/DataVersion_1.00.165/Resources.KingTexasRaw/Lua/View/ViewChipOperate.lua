...
function ViewChipOperate:setChipsInfo(left_chip,desk_top_chip,desk_bottom_chip,operate_type,param,action)
	...
	then
		title = self.ViewMgr.LanMgr:getLanValue("下注游戏", "BettingGame")
        tips_left = self.ViewMgr.LanMgr:getLanValue("牌桌下限", "LowerDesk")
        tips_right = self.ViewMgr.LanMgr:getLanValue("牌桌上限", "UpperDesk")
        self:setChips(desk_top_chip, desk_bottom_chip)
	elseif(self.ChipOperateType == CS.Casinos._eChipOperateType.Exchange)
	then
		title = self.ViewMgr.LanMgr:getLanValue("换钱", "ExchangeMoney")
        tips_left = self.ViewMgr.LanMgr:getLanValue("兑换下限", "LowerExchange")
        tips_right = self.ViewMgr.LanMgr:getLanValue("兑换上限", "UpperExchange")
        self:setChips(desk_top_chip, desk_bottom_chip)
	elseif(self.ChipOperateType == CS.Casinos._eChipOperateType.Transaction)
	then
		self.GTextFieldGetChipsTips.visible = true
        title = self.ViewMgr.LanMgr:getLanValue("交易筹码", "BargainingChip")
        tips_left = self.ViewMgr.LanMgr:getLanValue("交易下限", "LowerDeal")
        tips_right = self.ViewMgr.LanMgr:getLanValue("交易上限", "UpperDeal")
	end
...
function ViewChipOperate:setChips(desk_top_chip,desk_bottom_chip)
    ...
    self.GTextSelfLeft.text = self.ViewMgr.LanMgr:getLanValue("你的余额", "YourBalance") .. ":" .. UiChipShowHelper:getGoldShowStr(self.LeftGold,self.ViewMgr.LanMgr.LanBase)
