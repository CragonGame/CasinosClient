...
function ViewGiftShop:onCreate()
	ViewHelper:PopUi(self.ComUi,"礼物商城")
...
function ViewGiftShop:setGiftShopInfo(desktop_gift,from_etguid,to_etguid)
    ...
    for key,value in pairs(self.CasinosContext.TbDataMgrLua:GetMapData("ItemType")) do
		local tb_item = value
        if (tb_item.ParentTbId ~= self.mNotDesktopGiftTypeld and tb_item.ParentTbId ~= self.mDesktopGiftTypeld)
		then
		elseif((tb_item.ParentTbId == self.mNotDesktopGiftTypeld and desktop_gift == false) or (tb_item.ParentTbId == self.mDesktopGiftTypeld and desktop_gift))
		then
		else
...
function ViewGiftShopFactory:new(o,ui_package_name,ui_component_name,
