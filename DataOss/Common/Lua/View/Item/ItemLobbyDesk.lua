...
function ItemLobbyDesk:init()
    ...
    if (self.DeskTopInfo.list_seat_player ~= nil) then
...
function ItemLobbyDesk:NotBeCenter(usetween)
    ...
    if (usetween) then
        self.Item:TweenScale(CS.Casinos.LuaHelper.GetVector2(1, 1), 0.25)
    else
...
function ItemLobbyDesk:toBeCenter()
    ...
    self.Item:TweenScale(CS.Casinos.LuaHelper.GetVector2(1, 1) * 1.1, 0.25)
end
...
function ItemLobbyDesk:onClickSelf(e_c)
    if (e_c.inputEvent.isDoubleClick) then
        self:clickEnterDesk()
    end
end