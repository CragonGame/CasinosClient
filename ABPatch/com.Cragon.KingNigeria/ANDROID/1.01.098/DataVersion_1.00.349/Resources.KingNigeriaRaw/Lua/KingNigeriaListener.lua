...
function KingNigeriaListener:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
...
function KingNigeriaListener:onCreate()
    print("KingNigeriaListener:onCreate")
    self.ResourcesRowPathRoot = "Resources.KingNigeriaRaw/"
    ...
    for i = 0, list_db.Count - 1 do
        t_db[i] = CS.Casinos.CasinosContext.Instance.PathMgr:combinePersistentDataPath("Resources.KingNigeriaRaw/Config/" .. list_db[i] .. ".db")
    end
    ...
    self.ViewMgr:onCreate("Resources.KingNigeria/Ui/", "Resources.KingNigeriaRaw/")
...
function KingNigeriaListener:onDestroy()
    if (self.ViewMgr ~= nil)
    then
...
function KingNigeriaListener:onUpdate(tm)
    if (self.ViewMgr ~= nil)
    then
...
function KingNigeriaListener.OnApplicationPause(is_pause)
    print("OnApplicationPause   "..tostring(is_pause))
    if (is_pause == false)
    then
        local lis = KingNigeriaListener:new(nil)
...
function KingNigeriaListener:CreateController(table_data)

end

function KingNigeriaListener:_regLuaFilePath()
    ...
    casinos_context:regLuaFilePath("Resources.KingNigeriaRaw/Lua/View/", "ViewLogin", "ViewAbout", "ViewAgreeOrDisAddFriendRequest", "ViewBag", "ViewBank", "ViewChat"
    ...
    casinos_context:regLuaFilePath("Resources.KingNigeriaRaw/Lua/View/Item/", "ItemChatTargetInfo", "ItemChooseChatTargetInfo", "ItemDesktopHRewardPotPlayerInfo", "ItemGift", "ItemGiftType", "ItemInviteFriendPlay"
    ...
    casinos_context:regLuaFilePath("Resources.KingNigeriaRaw/Lua/View/Item/Mtt/", "ItemMatchType", "ItemMatchInfo","ItemBlind","ItemMatchInfoBlind","ItemSnowBallRewardInfo","ItemSnowBallRewardPlayerNum"
    ,"ItemMatchInfoReward","ItemMatchInfoRank")
end

function KingNigeriaListener:_regView()
...
function KingNigeriaListener:_regController()
...
function KingNigeriaListener:_addUiPackage()
...
function KingNigeriaListener:RegTbData()
...
function KingNigeriaListener:LoadDataDone()
...
function KingNigeriaListener:addUiPackage(name)
