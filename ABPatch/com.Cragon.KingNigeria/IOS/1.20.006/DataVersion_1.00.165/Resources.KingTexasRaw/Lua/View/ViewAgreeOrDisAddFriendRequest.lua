...
function ViewAgreeOrDisAddFriendRequest:onCreate()
	ViewHelper:PopUi(self.ComUi,"好友请求")
	...
	self.GBtnConfirm = self.ComUi:GetChild("ConfirmBtn").asButton
...
function ViewAgreeOrDisAddFriendRequest:addFriend(ev)
	...
	local s1,from_player_guid = ev.MapData:TryGetValue("FromPlayerGuid")
...
function ViewAgreeOrDisAddFriendRequest:setPlayerInfo(player_info)
    ...
    self.GTextNickName.text = player_info.PlayerInfoCommon.NickName
    self.GTextChips.text = UiChipShowHelper:getGoldShowStr(player_info.PlayerInfoMore.Gold, self.ViewMgr.LanMgr.LanBase)
    local level = CS.Casinos.CasinosContext.Instance:parseLanStr("等级:", "Level")
