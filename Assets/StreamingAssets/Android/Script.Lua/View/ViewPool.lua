...
function ViewPool:onCreate()
	...
	if (CS.Casinos.CasinosContext.Instance.Config.HaveMagicExp == true)
	then
...
function ItemMagicExpSenderPool:magicExpEnque(item)
	if (item.GCoMagicExpSender.displayObject ~= nil and item.GCoMagicExpSender.displayObject.gameObject ~= nil)
	then
...
function ItemShootingTextPool:shootingTextEnque(item)
	if (item.Com.displayObject ~= nil and item.Com.displayObject.gameObject ~= nil
			...
			and self.UiPool.ComUi.displayObject.gameObject ~= nil)
	then
