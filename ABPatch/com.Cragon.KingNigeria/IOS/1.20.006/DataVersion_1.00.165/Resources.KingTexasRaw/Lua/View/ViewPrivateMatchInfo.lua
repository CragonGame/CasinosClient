...
function ViewMatchInfo:onHandleEv(ev)
	...
	elseif(ev.EventName == "EvEntityResponseCancelSignupMatch")
	then
		if(ev.IsSucceed)
		then
...
function ViewPrivateMatchInfo:SetMatchInfo(match_info)
	...
	if(match_info.Info.Discribe ~= nil and string.len(match_info.Info.Discribe) > 0)
	then
	...
	-- 增购提示
	if(match_info.Info.CanAddonCount == 0)
	then
		self.GTextAddonTips = "无"
	else
		local temp = "第"..canRebuyMaxLevel + 1 .. "级;"
		local addonTimes = 0
		if(match_info.Info.RaiseBlindTbInfo.AddonScore % match_info.Info.InitScore == 0)
		then
	...
	for i = match_info.Info.RaiseBlindTbInfo.BeginId,match_info.Info.RaiseBlindTbInfo.EndId do
		...
		if(temp_level <= canRebuyMaxLevel)
		then
			can_rebuy = true
		end
		if(match_info.Info.CanAddonCount > 0 and temp_level == canRebuyMaxLevel + 1)
		then
			can_addon = true
		end
		local com = self.GListBlind:AddItemFromPool()
		ItemMatchInfoBlind:new(nil,com,blindInfo,temp_level,can_rebuy,can_addon,raiseBlindTimeMinute)
	end
	...
	if(self.IsSelfJoin)
	then
		if(nowTm < match_info.Info.DtMatchBegin)
		then
			self.GTextBtnSignupTitle.text = "取消报名"
			self.BtnSignupOption = BtnSignupOption.CancelSignup
		else
		...
		then
			self.GTextBtnSignupTitle.text = "报名"
			self.BtnSignupOption = BtnSignupOption.Signup
		elseif(nowTm >= match_info.Info.DtMatchBegin and nowTm < match_info.Info.DtSignupClose)
		then
	...
	if(self.ControllerMtt.Guid == match_info.Info.CreatePlayerGuid)
	then
...
function ViewPrivateMatchInfo:onClickBtnSignUp()
	if(self.BtnSignupOption == BtnSignupOption.Signup)
	then
		...
		ev.MatchGuid = self.MatchGuid
		self.ViewMgr:sendEv(ev)
	elseif(self.BtnSignupOption == BtnSignupOption.CancelSignup)
	then
		local msg_box = self.ViewMgr:createView("MsgBox")
	...
	elseif(self.BtnSignupOption == BtnSignupOption.Enter)
	then
