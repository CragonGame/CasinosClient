...
function ItemChatEx:setChat(sender_nameex, chat_contentex, sender_viplevel, sender_tm, sender_name_color_code, content_color_code, show_left,
    ...
    if (name_content_same_line == nil) then
        name_content_same_line = true
    end
    if (show_bg == nil) then
        show_bg = false
    end
    if (show_nickname == nil) then
        show_nickname = true
    end
    if (is_tmshow == nil) then
        is_tmshow = false
    end
    if (show_bg1 == nil) then
        show_bg1 = true
    end
    ...
    if (is_tmshow) then
        ...
        if (show_left == false) then
            group_parent = self.GGroupRight
        end
        if (show_left == true) then
    ...
    if (self.ControllerShowBg ~= nil) then
        if (show_bg) then
    ...
    if (sender_name_color_code ~= nil and string.len(sender_color_code) > 0) then
    ...
    if (content_color_code == nil or string.len(content_color_code) == 0) then
        content_color_code = "FFFFFF"
    end
    local chat_content = CS.ChatParser.inst:Parse("[color=#" .. content_color_code .. "]" .. chat_contentex .. "[/color]")
    if ((sender_tm_ex ~= nil and string.len(sender_tm_ex) > 0) or name_content_same_line == false) then
        if (show_nickname) then
    ...
    if (self.ItemType == CS.Casinos._eChatItemType.Marquee) then
