-- 会复制到各个项目中，实际调用的是各个项目中的ControllerLaunch
-- 和MainC配合使用
-- PreData下载后，下次启动才生效。放在Data目录中
...
function ControllerLaunch.onCreate()
    ...
    local predata_persistentdata_path = casinos_context.PathMgr:combinePersistentDataPath(
            casinos_context.ResourcesRowPathRoot .. "PreData/")
...
function ControllerLaunch.onUpdate(tm)
    ...
    then
        ...
        then
            ...
            -- ab全部下载完成后，lua call c#加载所有ab
            local view_preloading = launch.PreViewMgr.createView("PreLoading")
            view_preloading.setTip(tips)
            casinos_context.Listener:OnInitializeBegin(casinos_context.EbDataMgr,function ()
